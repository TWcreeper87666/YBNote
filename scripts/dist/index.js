...
/** 將實體放到玩家視線前方（編輯模式用） */
export function moveEntityToView(player, entity) {
...
// === 事件 ===
// 玩家開始使用物品
world.beforeEvents.itemUse.subscribe(async ({ source, itemStack: { typeId } }) => {
    ...
    if (hitSet.size > 0) {
        for (const id of hitSet) {
            if (!entityPlayedMap.get(id)) {
                ...
                const activeNotes = activeNotesByPlayer.get(player.id) ?? new Set();
                activeNotes.add(id);
                activeNotesByPlayer.set(player.id, activeNotes);
            }
        }
    ...
    for (const player of world.getAllPlayers()) {
        ...
        if (heldEntityId) {
            try {
                ...
                if (heldEntity)
                    moveEntityToView(player, heldEntity);
            }
            catch { }
        }
    }
