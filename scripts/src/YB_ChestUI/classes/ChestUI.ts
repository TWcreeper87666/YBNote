...
export class ChestUI {
  static config = {
    defaultPageName: "piano" as const,
  ...
  static getPageName(player: Player) {
    return (
      (player.getDynamicProperty("yb:eui_page") as string) ??
  ...
  static newUIItem(
    nameTag: string,
    typeId: string,
    options: { amount?: number; lore?: string[] } = {},
  ) {
  ...
  static setPageItem(
    player: Player,
    itemsWithIdx: { [key: number]: ItemStack },
  ) {
    const entity = this.getEntity(player);
    if (!entity) return;
  ...
  static #spawnEntity(player: Player, page = this.getPage(player)) {
    ...
    entity.nameTag =
      this.#getSizeName(page.size ?? this.config.defaultPageSize) +
      "§r" +
      (page.displayName ?? "ChestUI");
  ...
  static setData(
    player: Player,
    value: { [key: string]: boolean | number | string | object },
  ) {
  ...
  static update(player: Player) {
    ...
    if (!entity) entity = this.#spawnEntity(player, page);
    if (!entity) return;
    const container_e = entity.getComponent("inventory").container;
    // if (!container_e.isValid) return

    if (page.tickInterval) {
      const pageUpdateTick =
        (player.getDynamicProperty("yb:eui_pageUpdateTick") as number) ?? 0;
      if (system.currentTick >= pageUpdateTick) {
        player.setDynamicProperty(
          "yb:eui_pageUpdateTick",
          system.currentTick + page.tickInterval,
        );
  ...
  static init() {
    ...
    world.afterEvents.worldLoad.subscribe(() => {
      world
        .getAllPlayers()
        .forEach((player) =>
          ChestUI.setPage(player, ChestUI.config.defaultPageName),
        );
      world
        .getDimension("overworld")
        .getEntities({ type: "yb:ui_entity" })
        .forEach((entity) => ChestUI.removeUnownedEntity(entity));
    });

    world.afterEvents.playerInteractWithEntity.subscribe(
      ({ player, target, itemStack }) => {
        if (itemStack?.typeId !== "yb:eui_open") return;
