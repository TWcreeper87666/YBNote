import { Player, MolangVariableMap, system, Container, ItemStack, world } from "@minecraft/server";
...
function getSoundLore(soundIdx: number) {
    const copy = [...sounds]
...
ChestUI.setUIPage('piano', new Page(pageInit(), {
    ...
    start: ({ player, container_e }) => {
        ...
        ChestUI.setData(player, { pitchIdx, soundIdx } as NoteData)
    }
}))
...
function updateSound(player: Player, data: NoteData, container: Container, playSound = true) {
    if (playSound) player.playSound(sounds[data.soundIdx])
    for (let i = 36; i <= 38; i++) {
...
function updateFunc({ player, idx }: { player: Player, idx: number }) {
    const data = ChestUI.getData(player) as NoteData
