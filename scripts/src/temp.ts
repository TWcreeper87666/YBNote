import { Entity, MolangVariableMap, Player, system, world } from '@minecraft/server'
...
export const PDP_tmpSoundIdx = new PlayerDP<number>('note_tmpSoundIdx', sounds.length - 1)
export const EDP_pitchIdx = new EntityDP<number>('note_pitchIdx', 12)
export const EDP_sound = new EntityDP<string>('note_sound', 'note.harp')

// Map 改寫後
export const playerUsingPlayMap = new Map<string, boolean>()
export const playerFacingEntityMap = new Map<string, string>()
export const entityPlayedMap = new Map<string, boolean>()
export const entityHoldingMap = new Map<string, boolean>()

const playingNoteMap = new Map<string, Set<string>>()
const holdingNoteMap = new Map<string, string>()

let circle_mvm: MolangVariableMap
let note_mvm: MolangVariableMap
let facing_circle_mvm: MolangVariableMap
let particle_mvm: MolangVariableMap
let particle2_mvm: MolangVariableMap
...
function update() {
    ...
    world.getAllPlayers().forEach(player => {
        ...
        const hitEntitySet = new Set<string>()
        const playing = playingNoteMap.get(player.id) ?? new Set<string>()
        ...
        for (const { entity } of hits) {
            ...
            if (!usingPlay || entityPlayedMap.get(entity.id)) continue
        ...
        if (holdingNote) {
            ...
            try { world.getEntity(holdingNote).teleport(vectorAdd(head, view)) } catch { }
        }

        for (const id of playing) {
            if (hitEntitySet.has(id) && usingPlay) continue
